...
const progressSchema = z
  .object({
    ...
    eventId: z.string().trim().min(16).max(100).regex(/^[A-Za-z0-9._:-]+$/),
  })
  .strict();
const noteSchema = z
  .object({
    sessionId: entityIdSchema,
    timestampSeconds: z.number().finite().min(0).max(24 * 60 * 60),
...
function accessProjection(registration: {
  status: string;
...
async function findOwnedSession(
  context: Awaited<ReturnType<typeof requireViewerContext>>,
  sessionId: string,
) {
...
viewerAccountRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.setHeader('Cache-Control', 'private, no-store');
    res.json({ ok: true, roomUrl: buildFrontendUrl('/crisis_premium/webinar.html'), correlationId: accountCorrelationId() });
  }),
);
...
viewerAccountRouter.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.status(200).json({ ok: true, savedAt: favorite.createdAt.toISOString(), correlationId: accountCorrelationId() });
  }),
);
      ...
      if (marketingEmailChanged || marketingTelegramChanged) {
        ...
        await tx.lead.update({
          ...
          data: {
            ...
            marketingConsentAt: nextMarketingEnabled
              ? previousMarketingEnabled
                ? undefined
                : now
              : null,
          },
