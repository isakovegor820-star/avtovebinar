...
Статус после этапа Security + Access: часть P0 закрыта отдельным hardening-шагом. Replay переведен на 7 дней, production guard усилен против email log/test-room, token storage во вкладках ограничен dev/local режимом, новые room/reminder tokens получают `expiresAt`.
