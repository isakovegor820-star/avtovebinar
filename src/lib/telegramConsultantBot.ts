...
import {
  classifyTelegramConsultantText,
  recordTelegramConsultantMessage,
} from './tenancy/telegramConsultant.js';
...
function buildContactRows(update: ConsultantTelegramUpdate, text: string, classification?: ReturnType<typeof classifyTelegramConsultantText>) {
  ...
  return [
    ...
    classification ? `Тема: ${classification.topic}; намерение: ${classification.intent}; срочность: ${classification.urgency}` : null,
    '',
    `Сообщение: ${text}`,
  ].filter(Boolean);
}
...
async function notifyTenantManagers(
  record: Awaited<ReturnType<typeof recordTelegramConsultantMessage>>,
) {
  const scope = record.scope;
  if (!scope.organizationId || !scope.registrationId || !scope.crmContactId || !scope.webinarId || !scope.webinarSessionId) {
    return 0;
  }
