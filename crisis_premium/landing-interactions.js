try {
  localStorage.removeItem('aspbWebinarTargetMsk');
} catch {}
...
function initMobileDetails() {
  const toggle = document.getElementById('mobileDetailsToggle');
  const label = toggle?.querySelector('[data-mobile-details-label]');
  const mobile = window.matchMedia('(max-width: 767px)');
  if (!toggle || !label) return;

  const setExpanded = expanded => {
    document.body.classList.toggle('mobile-details-collapsed', mobile.matches && !expanded);
    toggle.setAttribute('aria-expanded', String(expanded));
    label.textContent = expanded ? 'Скрыть подробности' : 'Показать программу, расчёты и команду';
  };

  const revealForHash = () => {
    const target = window.location.hash ? document.querySelector(window.location.hash) : null;
    if (target?.closest('.mobile-detail-section')) setExpanded(true);
  };
  ...
  document.querySelectorAll('a[href^="#"]').forEach(link => {
    link.addEventListener('click', event => {
      ...
      // Wait for the collapsed mobile section to participate in layout before
      // calculating its scroll position. One handler now owns reveal, history and
      // scrolling, so a hidden target cannot be measured by an earlier listener.
