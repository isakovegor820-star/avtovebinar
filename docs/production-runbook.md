...
Перед созданием или восстановлением контура запустите ручной CI workflow с
`deploy_target=staging-inventory`. Этот режим только читает безопасные признаки:
hostname origin, наличие staging/test-маркера в имени БД, занятость
кандидатного host-порта 5176, доступность Docker/Nginx/Certbot и список только
АСПБ-контейнеров. Значение `DATABASE_URL`, credentials и остальные env-поля не
выводятся; checkout, миграции и перезапуск сервисов не выполняются.
