...
import { z } from 'zod';
import { MARKETING_EMAIL_CONSENT, MARKETING_TELEGRAM_CONSENT, consentDocumentHash } from '../consentDocuments.js';
...
function deliveryProjection(delivery: any, canRetry: boolean) {
  return {
    ...
    canRetry: canRetry && ['RETRY_SCHEDULED', 'BLOCKED', 'DEAD_LETTER', 'CANCELLED'].includes(delivery.status),
  };
}
    ...
    Promise.all(
      contact.registrations.map(async registration => {
        ...
        return {
          ...
          email: {
            allowed: Boolean(email?.allowed),
            reasonCode: email?.reasonCode ?? 'crm_delivery_recipient_unavailable',
          },
          telegram: {
  ...
  return db.$transaction(async tx => {
    ...
    if (!['RETRY_SCHEDULED', 'BLOCKED', 'DEAD_LETTER', 'CANCELLED'].includes(existing.status)) {
      throw new AppError(
        ...
        undefined,
        'crm_delivery_retry_not_allowed',
      );
    }
    const eligibility = await evaluateDeliveryEligibility(
      tx,
