...
const APPROVED_TEMPLATE_VARIABLES = ['participant_name', 'webinar_title', 'session_datetime', 'room_link'] as const;
...
function parseTemplateVariables(text: string) {
  ...
  if (
    withoutApproved.includes('{{') ||
    withoutApproved.includes('}}') ||
    variables.some(value => !approvedVariables.has(value))
  ) {
    throw new AppError(
      400,
      'Шаблон содержит неизвестную или некорректную переменную',
      undefined,
      'telegram_template_variable_invalid',
    );
  }
...
function validatePublishableTemplate(text: string) {
  ...
  if (!variables.includes('room_link')) {
    throw new AppError(
      400,
      'Опубликованный шаблон должен содержать {{room_link}}',
      undefined,
      'telegram_template_link_required',
    );
  }
  return variables;
}
...
async function collectTenantBroadcastCandidates(
  ...
  if (registrations.length > MAX_TENANT_BROADCAST_RECIPIENTS) {
    throw new AppError(
      409,
      'Сегмент превышает безопасный лимит рассылки',
      undefined,
      'tenant_telegram_segment_too_large',
    );
  }
...
function tenantBroadcastSnapshotHash(candidates: TenantBroadcastCandidate[]) {
  ...
  for (const candidate of candidates) {
    const chatHash = crypto
      ...
      .update(`telegram-broadcast-chat:v1:${candidate.chatId}`)
      .digest('hex');
    digest.update(
      [
...
export async function createTenantTelegramBroadcastTemplate(db: PrismaClient, context: TenantContext, input: unknown) {
  ...
  return db.$transaction(
    async tx => {
      ...
      if (candidates.length !== preview.total || tenantBroadcastSnapshotHash(candidates) !== preview.snapshotHash) {
        throw new AppError(
          409,
          'Состав сегмента изменился. Выполните preview повторно.',
          undefined,
          'tenant_telegram_preview_stale',
        );
      }
...
async function mutateTenantBroadcastControl(
  ...
  return db.$transaction(async tx => {
    ...
    const job = await tx.telegramBroadcastJob.findFirst({
    ...
    await tx.auditLog.create({
      data: {
        ...
        afterJson: {
          ...
          requested: Boolean(updated.pauseRequestedAt || updated.cancelRequestedAt),
        },
      },
    });
