...
const CRM_READ_ROLES = ['OWNER', 'CRM_MANAGER', 'ANALYST', 'AUDITOR'] as const satisfies readonly OrganizationMembershipRole[];
...
const bulkActionSchema = z.discriminatedUnion('type', [
  ...
  z.object({ type: z.literal('CHANGE_STAGE'), stageId: idSchema, reason: z.string().trim().min(3).max(1_000).optional() }).strict(),
...
const bulkExecuteSchema = z
  .object({ mode: z.literal('EXECUTE'), previewId: idSchema })
  .strict();
...
function hasCrmExportPermission(context: TenantContext) {
  if (!context.permissions || typeof context.permissions !== 'object' || Array.isArray(context.permissions)) return false;
  const crm = (context.permissions as Record<string, unknown>).crm;
  return Boolean(crm && typeof crm === 'object' && !Array.isArray(crm) && (crm as Record<string, unknown>).export === true);
}
...
function assertReminderOrder(reminderAt: Date, dueAt: Date) {
  if (reminderAt.getTime() > dueAt.getTime()) {
    throw new AppError(
      400,
      'Напоминание должно быть не позже срока задачи',
      undefined,
...
async function ensureStageForLegacyCode(tx: CRMTransaction, pipeline: Awaited<ReturnType<typeof ensureDefaultPipeline>>, code: string) {
  ...
  return tx.cRMStage.create({
    data: {
      ...
      code,
      name: code,
      semanticCategory: code === 'paid' || code === 'won' ? 'WON' : code === 'lost' || code === 'not_target' ? 'LOST' : 'OPEN',
...
export async function linkVerifiedRegistrationToCrm(
  tx: CRMTransaction,
  registrationId: string,
  now = new Date(),
) {
...
function scoreDetailProjection(contact: any) {
  ...
  const groups = new Map<string, { code: string; label: string; pointsEach: number; count: number; lastOccurredAt: Date }>();
...
async function validateBulkActionTarget(
  ...
  if (action.type === 'ASSIGN_MANAGER' || action.type === 'CREATE_TASK') {
    const membershipId = action.type === 'ASSIGN_MANAGER' ? action.assigneeMembershipId : action.task.assigneeMembershipId;
...
export async function previewCrmBulkAction(
  db: PrismaClient,
  context: TenantContext,
  input: unknown,
  now = new Date(),
  ...
  return db.$transaction(async tx => {
    ...
    if (contacts.length > CRM_BULK_MAX_CONTACTS) {
      throw new AppError(400, `Массовое действие ограничено ${CRM_BULK_MAX_CONTACTS} контактами`, undefined, 'crm_bulk_selection_too_large');
    }
...
export async function executeCrmBulkAction(
  db: PrismaClient,
  context: TenantContext,
  input: unknown,
  now = new Date(),
  ...
  for (const contactId of contactIds) {
    try {
      if (action.type === 'ASSIGN_MANAGER') {
        ...
        await transitionCrmContactStage(db, context, contactId, { stageId: action.stageId, reason: action.reason }, now);
      } else {
...
export async function exportCrmContacts(
  ...
  if (contacts.length > CRM_EXPORT_MAX_CONTACTS) {
    throw new AppError(400, `Экспорт ограничен ${CRM_EXPORT_MAX_CONTACTS} контактами`, undefined, 'crm_export_too_large');
  }
  const masked = isMaskedRole(context);
  const header = ['contact_id', 'name', 'email', 'phone', 'stage_code', 'stage', 'manager', 'score', 'hot', 'tags', 'next_contact_at', 'source'];
...
export async function getCrmContact(db: PrismaClient, context: TenantContext, contactIdInput: unknown) {
  ...
  const userIds = [...new Set(contact.registrations.map(registration => registration.userId).filter(Boolean))] as string[];
  ...
  for (const progress of progressRows) {
    const percent = progress.durationMs ? Math.min(100, Math.round((progress.positionMs / progress.durationMs) * 100)) : null;
    addTimelineItem(timeline, {
  ...
  timeline.sort((left, right) => right.occurredAt.getTime() - left.occurredAt.getTime() || right.id.localeCompare(left.id));
  ...
  return {
    ...
    registrations: contact.registrations.map(registration => ({
      ...
      deliveryEligibility:
        deliveryOverview.eligibility.find(item => item.registrationId === registration.id) ?? {
          registrationId: registration.id,
