...
describe('critical path integration scenarios', () => {
  it('runs the full critical path integration scenario', async () => {
    // 0. Seed a default admin user and a future webinar session so that registration works
    ...
    tomorrow.setUTCHours(8, 0, 0, 0); // 11:00 Moscow time

    await prisma.webinarSession.create({
      data: {
        title: 'Антикризисный вебинар',
    ...
    const adminRegsResponse = await request(app)
