...
export type PublicChatMessageType = 'participant' | 'moderator' | 'prepared_question' | 'ai_moderator' | 'system';

const HTML_TAG_PATTERN = /<\s*\/?\s*[a-z][^>]*>/iu;
const ACTIVE_CONTENT_PATTERN = /(?:javascript\s*:|data\s*:\s*text\/html)/iu;
const SYNTHETIC_ONLINE_COUNT_PATTERN =
  /(?:\d{1,7}\s*(?:человек|участник(?:ов|а)?|зрител(?:ей|я))\s*(?:онлайн|смотр)|онлайн\s*[:—-]?\s*\d{1,7})/iu;
const SYNTHETIC_TESTIMONIAL_PATTERN =
  /(?:(?:мой|наш)\s+отзыв|(?:я|мы)\s+(?:уже\s+)?(?:заработал(?:а|и)?|получил(?:а|и)?\s+(?:результат|выплату|деньги)|списал(?:а|и)?\s+долг|выиграл(?:а|и)?\s+(?:дело|суд)))/iu;
const PERSONAL_CONTEXT_PATTERN =
  /(?:\b(?:мне|нам|мой|моя|моё|мои|моего|моей|моём|нашей|нашего)\b|\bу\s+(?:меня|нас)\b|\bв\s+(?:моей|нашей)\s+ситуации\b)/iu;
const INDIVIDUAL_ACTION_PATTERN =
  /(?:что\s+(?:мне|нам)\s+делать|как\s+(?:мне|нам)\s+(?:поступить|действовать)|(?:можно|стоит|следует)\s+ли\s+(?:мне|нам)|(?:посоветуй(?:те)?|дайте\s+совет)|(?:оцените|проанализируйте|проверьте)\s+(?:мой|мою|моё|нашу|наш))/iu;
const PERSONAL_LEGAL_OBJECT_PATTERN =
  /\b(?:договор|иск|жалоб|суд|долг|банкротств|налог|штраф|увольнен|наследств|сделк|претензи|постановлен|исполнительн)\w*/iu;
...
export function sanitizeParticipantQuestion(input: string) {
  ...
  const safeText = stripUnsafeControls(normalized)
    .replace(/\n{3,}/gu, '\n\n')
    .trim();

  if (HTML_TAG_PATTERN.test(safeText) || ACTIVE_CONTENT_PATTERN.test(safeText)) {
    throw new AppError(400, 'Вопрос должен содержать только обычный текст', undefined, 'chat_markup_not_allowed');
  }
