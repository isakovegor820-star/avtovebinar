...
const chatIdSchema = z.string().trim().regex(/^-?\d{1,20}$/);
...
function signCallbackRecord(record: {
  ...
  return crypto
    ...
    .update(
      [record.id, record.organizationId, record.webinarId, record.webinarSessionId, record.action, record.expiresAt.toISOString()].join('|'),
    )
    .digest('base64url')
    .slice(0, 16);
}
...
export async function createTelegramManagerBinding(
  ...
  return db.$transaction(async tx => {
    ...
    const existing = await tx.telegramManagerChatBinding.findFirst({
      where: { organizationId: context.organizationId, membershipId, status: { in: ['PENDING_CHAT', 'PENDING_OWNER', 'ACTIVE'] } },
      include: { membership: { include: { user: { select: { displayName: true } } } } },
    });
    if (existing) {
      throw new AppError(409, 'Для менеджера уже есть незавершённая или активная привязка', undefined, 'telegram_manager_binding_exists');
    }
  ...
  return db.$transaction(async tx => {
    ...
    if (data.action === 'CHANGE_STAGE') {
      ...
      const stage = await tx.cRMStage.findFirst({
        where: { id: stageId, organizationId: context.organizationId, pipelineId: registration.crmContact.pipelineId, status: 'ACTIVE' },
      });
      if (!stage) bindingUnavailable();
    }
    const duplicate = await tx.telegramManagerCallback.findUnique({
      where: { organizationId_idempotencyKey: { organizationId: context.organizationId, idempotencyKey: data.idempotencyKey } },
    });
    if (duplicate) {
      if (
        ...
        throw new AppError(409, 'Idempotency key is already used', undefined, 'telegram_manager_callback_idempotency_conflict');
      }
      return { callbackId: duplicate.id, callbackData: callbackDataFor(duplicate), expiresAt: duplicate.expiresAt, replayed: true };
    }
...
export async function executeTelegramManagerCallback(
  ...
  return db.$transaction(async tx => {
    ...
    if (!callback || !timingSafeTextEqual(signCallbackRecord(callback), submittedSignature)) return callbackUnavailable();
    ...
    if (callback.action === 'ACCEPT_CONTACT') {
      resultCode = callback.crmContact.ownerMembershipId === membership.id ? 'contact_already_accepted' : 'contact_accepted';
