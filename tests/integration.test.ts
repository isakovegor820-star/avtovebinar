...
describe('tenant chat moderation batch', () => {
  ...
  it('keeps approved chat, spam controls and moderator actions exact-session and tenant-scoped', async () => {
    ...
    const foreignRead = await foreignModerator.agent.get(
      `/api/v1/moderation/sessions/${session.id}/messages`,
    );
    ...
    const forgedScope = await moderator.agent
      ...
      .send({ action: 'HIDE', reason: 'Персональные данные', expectedRevision: 0, organizationId: foreignTenant.organization.id });
    ...
    await expect(
      prisma.auditLog.count({
        where: {
          organizationId: tenant.organization.id,
          action: {
            in: ['chat.message.hidden', 'chat.message.restored', 'chat.registration.blocked', 'chat.registration.restored'],
          },
        },
      }),
    ).resolves.toBe(4);
  ...
  it('grounds moderator drafts, blocks personalized advice and synchronizes question queues with CRM', async () => {
    ...
    expect(source.body.suggestion).toMatchObject({
      outcome: 'GROUNDED',
      grounding: { type: 'source', title: 'Порядок подачи заявления о несостоятельности', url: 'https://pravo.gov.ru/' },
    });
    ...
    expect(prioritized.body.question).toMatchObject({ moderationStatus: 'ACTION_REQUIRED', priority: 'HIGH', moderationRevision: 2 });
    ...
    expect(repeatingQueue.body.questions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ id: personalQuestion.id, repeatCount: 2 }),
      ]),
    );
...
describe('tenant CRM contact and pipeline batch', () => {
  it('keeps contact, filters, timeline and stage writes tenant-scoped and auditable', async () => {
    ...
    await expect(
      prisma.cRMContact.findUniqueOrThrow({ where: { id: contact!.id } }),
    ).resolves.toMatchObject({ score: 110 });
    await expect(
    ...
    await expect(
      prisma.cRMScoringRuleSet.count({ where: { organizationId: tenantA.organization.id } }),
    ).resolves.toBe(2);
    await expect(
      prisma.cRMContact.findUniqueOrThrow({ where: { id: contact!.id } }),
    ).resolves.toMatchObject({ score: 5, manualHot: false });
  ...
  it('rechecks tenant channel consent at enqueue and send while exposing safe retry/dead-letter state', async () => {
    ...
    await expect(prisma.cRMDelivery.findUniqueOrThrow({ where: { id: queuedEmail.body.delivery.id } })).resolves.toMatchObject({
      status: 'SENT',
      attempts: 1,
    ...
    await expect(prisma.cRMDelivery.findUniqueOrThrow({ where: { id: logModeEmail.body.delivery.id } })).resolves.toMatchObject({
      status: 'CANCELLED',
...
describe('tenant Telegram manager bot foundation', () => {
  it('requires owner-confirmed chat binding and executes signed exact-scope callbacks idempotently', async () => {
    ...
    await expect(
      prisma.auditLog.count({ where: { organizationId: tenantA.organization.id, action: { startsWith: 'telegram.' } } }),
...
describe('tenant Telegram broadcast flow', () => {
  it('requires a published safe template and expiring preview before exact-scope queueing', async () => {
    ...
    expect(foreignPublish.body).toMatchObject({ code: 'tenant_telegram_template_unavailable', error: missingPublish.body.error });
    ...
    await expect(
      prisma.registrationToken.count({ where: { registrationId: registrations[0].registrationId, purpose: 'registration' } }),
    ).resolves.toBe(1);
    await expect(
      prisma.registrationToken.count({ where: { registrationId: registrations[1].registrationId, purpose: 'registration' } }),
    ).resolves.toBe(0);
    ...
    expect(cancelled.body.job).toMatchObject({ status: 'cancelled', cancelReason: 'Рассылка отменена владельцем после проверки' });
    await expect(
    ...
    expect(foreignPause.body).toMatchObject({ code: 'tenant_telegram_broadcast_unavailable', error: missingPause.body.error });
    ...
    await expect(
      prisma.auditLog.count({ where: { organizationId: tenantA.organization.id, action: { startsWith: 'telegram.broadcast.' } } }),
...
describe('critical path integration scenarios', () => {
  ...
  it('scopes participant bot commands to current accessible Webinar sessions', async () => {
    ...
    await expect(
      prisma.telegramBotEvent.count({ where: { organizationId: tenantB.organization.id } }),
    ).resolves.toBe(0);
    ...
    await expect(
      prisma.consentRecord.count({
        where: { leadId: lead.id, kind: 'marketing_telegram', action: 'revoke', sourceForm: 'telegram_participant_bot' },
      }),
    ).resolves.toBe(1);
  }, 60_000);
  ...
  it('registers the shown tenant session and keeps the viewer account private, scoped and idempotent', async () => {
    ...
    await expect(
      prisma.registration.count({ where: { webinarSessionId: firstSession.id } }),
    ).resolves.toBe(1);
    await expect(
    ...
    await expect(
      prisma.registration.count({ where: { webinarSessionId: foreignSession.id } }),
    ).resolves.toBe(0);
    ...
    await expect(
      prisma.viewerWebinarFavorite.count({
        where: { userId: storedRegistration.userId!, organizationId: tenant.organization.id, webinarId: firstWebinar.id },
      }),
    ).resolves.toBe(1);
    ...
    await expect(
      prisma.viewerWebinarProgress.count({ where: { webinarSessionId: firstSession.id } }),
    ).resolves.toBe(1);
    ...
    const unknownProgressRead = await viewer.get(
      '/api/v1/viewer/progress/00000000-0000-4000-8000-000000000000',
    );
    ...
    const deletedNote = await viewer
      .delete(`/api/v1/viewer/notes/${note.body.note.id}`)
      .set('x-csrf-token', csrfToken);
