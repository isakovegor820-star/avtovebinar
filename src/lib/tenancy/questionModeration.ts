...
import {
  ...
  requireCurrentModeratorMembership,
...
export const generateQuestionSuggestionSchema = z
...
export async function listModerationQuestions(
  ...
  const fingerprints = questions.map(question => question.textFingerprint).filter((value): value is string => Boolean(value));
...
async function groundedContent(tx: QuestionTransaction, question: ScopedQuestion): Promise<SuggestionContent & { transcriptId: string | null }> {
  if (classifyLegalAdviceRequest(question.text) === 'PERSONALIZED_LEGAL_ADVICE') {
    return {
      answer: 'Этот вопрос требует оценки индивидуальной ситуации. AI-модератор не формирует персональную юридическую рекомендацию; передайте вопрос автору.',
  ...
  if (transcriptHit) {
    ...
    const excerpt = transcriptHit.text.length > 360 ? `${transcriptHit.text.slice(0, 357).trim()}…` : transcriptHit.text;
    return {
  ...
  return {
    answer: 'В опубликованной расшифровке и разрешённых источниках нет достаточного основания для ответа. Передайте вопрос автору.',
    outcome: 'NO_BASIS',
    handoffRequired: true,
    grounding: null,
    transcriptId: null,
