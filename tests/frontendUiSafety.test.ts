...
describe('frontend player safety regressions', () => {
  ...
  it('keeps viewer progress foreground-only and renders account data without HTML injection', () => {
    ...
    expect(catalogDetailHtml).toContain('id="detailRegistrationStatus" class="catalog-registration-status" aria-live="polite"');
  ...
  it('keeps the tenant CRM URL-addressable, role-aware and safe to render', () => {
    ...
    expect(crmJs).toContain("await post(`/v1/crm/contacts/${encodeURIComponent(contactId)}/tags/${encodeURIComponent(tagId)}`, {})");
    expect(crmJs).toContain("await deleteJson(`/v1/crm/contacts/${encodeURIComponent(contactId)}/tags/${encodeURIComponent(tagId)}`)");
    ...
    expect(crmHtml).toContain('id="crmBulkPreviewStatus" class="crm-live-status" aria-live="polite" aria-atomic="true"');
  ...
  it('labels synthetic chat and keeps tenant moderation reasoned, accessible and injection-safe', () => {
    ...
    expect(moderationHtml).toContain('id="moderationMessageList" class="mt-6 grid gap-4" aria-live="polite" aria-busy="false"');
