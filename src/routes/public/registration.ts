...
registrationRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await prisma.$transaction(async tx => {
        ...
        lead = await tx.lead.update({
          ...
          data: {
            ...
            ...(data.marketingEmailConsent
              ? { marketingConsent: true, marketingConsentAt: now, marketingEmailConsent: true, marketingEmailConsentAt: now }
              : {}),
      ...
      if (shouldEnqueueEmailOutbox()) {
        const enqueue = alreadyActive && !issueParticipantSession ? enqueueParticipantLoginEmail : enqueueRegistrationEmail;
        await enqueue(tx, {
          registrationId: registration.id,
          webinarSessionId: target.id,
          toEmail: lead.email,
